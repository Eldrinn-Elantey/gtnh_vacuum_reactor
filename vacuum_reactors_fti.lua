...
local function create_widgets(lsc, reactors)
    ...
    local lsc_widget = {
        ...
        draw = function(widget, tick)
            ...
            -- Отрисовка строки I/O [EU/t]
            local x = widget.min_x + 2
            local y = widget.min_y + 4
            gpu.setForeground(0xFFFFFF)  -- Стартовый белый цвет
                    
            -- Базовый текст
            gpu.set(x, y, "I/O [EU/t]: ")
            x = x + string.len("I/O [EU/t]: ")
                    
            -- Ввод (всегда положительный, зеленый)
            gpu.setForeground(0x00FF00)
            local input_str = "+" .. format_number(lsc_status.avg_input_eut)
            gpu.set(x, y, input_str)
            x = x + string.len(input_str)
                    
            -- Вывод (красный "-" и значение)
            gpu.setForeground(0xFF0000)
            local output_str = " - " .. format_number(math.abs(lsc_status.avg_output_eut))
            gpu.set(x, y, output_str)
            x = x + string.len(output_str)
                    
            -- Потери (красный "-" и значение)
            gpu.setForeground(0xFF0000)
            local loss_str = " - " .. format_number(math.abs(lsc_status.passive_loss_eut))
            gpu.set(x, y, loss_str)
            x = x + string.len(loss_str)
                    
            -- Итоговое значение (динамический цвет)
            gpu.setForeground(0xFFFFFF)
            gpu.set(x, y, " = ")
            x = x + 3  -- Длина строки " = "
            local net_color = average_net_input_eut >= 0 and 0x00FF00 or 0xFF0000
            gpu.setForeground(net_color)
            gpu.set(x, y, format_number(average_net_input_eut))
            x = x + string.len(format_number(average_net_input_eut))
                    
            -- Завершение строки
            gpu.setForeground(0xFFFFFF)
            gpu.set(x, y, " EU/t" .. empty_or_full_message)
    ...
    local reactors_widget = {
        ...
        draw = function(widget, tick)
            ...
            local reactor_display_format = " %s | %8s   | %6sEU/t │ %3d%% │ %s"
            ...
            for _, reactor in ipairs(reactors) do
                ...
                local format_eut = format_number(eut)
                ...
                gpu.set(widget.min_x, yy, string.format(reactor_display_format, enabled, transposer_uuid8, format_eut, heat_pct, status))
